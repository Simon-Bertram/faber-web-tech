...
- **For one-off utilities and scaffolding:** Use `pnpm dlx` (e.g., `pnpm dlx create-next-app@latest my-app`, `pnpm dlx knip`).
...
- **For running dependencies already in `package.json`:** Use `pnpm <cmd>` or `pnpm exec <cmd>` (e.g., `pnpm eslint .` or `pnpm exec eslint .`).
...
- **When to use `npx`:** Only fall back to `npx` if a specific tool explicitly requires npm's environment or if you are running in an environment where pnpm is not installed.
